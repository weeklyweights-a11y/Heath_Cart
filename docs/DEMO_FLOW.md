# HealthCart Demo Flow — 15 Steps

Run on **desktop (1024px+)** and **mobile (375px)**. Mark pass/fail.

| # | Step | Desktop | Mobile |
|---|------|:-------:|:------:|
| 1 | Open app → landing page loads | [ ] | [ ] |
| 2 | Click "Try the Demo" → Johnson family created, shop opens | [ ] | [ ] |
| 3 | Browse — kale green (Mike), peanut butter red (Jake), quinoa green (Linda) | [ ] | [ ] |
| 4 | Open kale — USDA nutrition, fiber highlighted, full reasoning | [ ] | [ ] |
| 5 | Chat: Linda visiting + Jake cold + hot weather + BBQ message | [ ] | [ ] |
| 6 | AI responds naturally, acknowledging everything | [ ] | [ ] |
| 7 | Shop transforms (desktop behind panel; mobile after closing chat) | [ ] | [ ] |
| 8 | Ask: "Can Jake have trail mix with his cold?" — allergy + cold advice | [ ] | [ ] |
| 9 | Open basket — items, ~87% coverage, ~$84.50 total | [ ] | [ ] |
| 10 | Per-member coverage: Mike, Jake, Linda, Sarah percentages | [ ] | [ ] |
| 11 | Change kale 1 lb → 2 lb — live coverage update | [ ] | [ ] |
| 12 | Set budget $75 — re-optimize + tradeoffs | [ ] | [ ] |
| 13 | Chat: "Actually Jake is feeling better" — store + basket update | [ ] | [ ] |
| 14 | Chat: "Linda left yesterday" — store adjusts for 3 people | [ ] | [ ] |
| 15 | Spanish message test — extraction responds appropriately | [ ] | [ ] |

## Video script (under 3 minutes)

| Time | Content |
|------|---------|
| 0:00–0:20 | Intro: HealthCart — grocery store enhanced by AI |
| 0:20–0:40 | Johnson family demo quick-start |
| 0:40–1:20 | Store scoring: kale, quinoa, peanut butter |
| 1:20–2:00 | Natural language → store transforms |
| 2:00–2:30 | Basket, budget, live coverage |
| 2:30–2:50 | Tech stack |
| 2:50–3:00 | Closing |

**YouTube (unlisted):** _paste link here_
